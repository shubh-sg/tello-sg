...
def findFaces(img, imgGray, face_cascade):
...
def findHands(img, imgGray, hand_cascade):
        return img, 0
...
def processFrame(img):
        ...
        img, face_info = findFaces(img, imgGray, face_cascade)
        img, hand_info = findHands(img, imgGray, hand_cascade)               
...
def displayText(img, commands, face_info, hand_info):
        ...
        global HEIGHT, WIDTH
        cv2.arrowedLine(img, (WIDTH//2, HEIGHT//2), (face_info[0][0], face_info[0][1]), (200, 0, 100), 3)
        cv2.putText(img, 'commands : {}'.format(commands),  (30,30),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
        cv2.putText(img, 'x : {} y : {}'.format(cx, cy),  (30,60),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
        cv2.putText(img, 'area : {}'.format(area),  (30,90),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
        ...
        if fb > 0:      fb = 'forward'
        elif fb < 0:    fb = 'backward'
        if ud > 0:      ud = 'up'
        elif ud < 0:    ud = 'down'
        if yv > 0:      yv = 'cw'
        elif yv < 0:    yv = 'ccw'

        cv2.putText(img, 'drone moves : {}'.format(lr),  (30,200),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
        cv2.putText(img, 'drone moves : {}'.format(fb),  (30,230),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
        cv2.putText(img, 'drone moves : {}'.format(ud),  (30,260),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
        cv2.putText(img, 'drone rotates : {}'.format(yv),  (30,290),  cv2.FONT_HERSHEY_COMPLEX,  0.5,  (0, 0, 255),  2)
